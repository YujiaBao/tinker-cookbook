...
class RunRegistry:
    ...
    def get_eval_store(self) -> Any | None:
        """Get the EvalStore if eval data exists.

        Returns None if no eval data found. Import is deferred to avoid
        circular deps (EvalStore is in stores/eval_store.py, added in Phase 2).
        """
        for storage in self._storages:
            for prefix in ["eval", "eval_store", ""]:
                ...
                if storage.exists(path):
                    try:
                        ...
                        return EvalStore(storage, prefix)
                    except ImportError:
                        # eval_store not yet implemented
                        return None
        return None

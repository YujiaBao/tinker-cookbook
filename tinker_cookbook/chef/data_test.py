...
class TestTimingReader:
    ...
    def test_build_span_tree(self, tmp_run: Path) -> None:
        reader = TimingReader(LocalStorage(tmp_run), "timing_spans.jsonl")
        reader.read()
        tree = reader.build_span_tree(0)
        assert tree["step"] == 0
        assert tree["root"] is not None
        assert tree["root"]["name"] == "iteration"
        assert tree["total_duration"] > 0
        # Should have children (the top-level spans)
        assert len(tree["root"]["children"]) > 0

    def test_build_span_tree_empty(self, tmp_run: Path) -> None:
        reader = TimingReader(LocalStorage(tmp_run), "timing_spans.jsonl")
        reader.read()
        tree = reader.build_span_tree(999)  # nonexistent step

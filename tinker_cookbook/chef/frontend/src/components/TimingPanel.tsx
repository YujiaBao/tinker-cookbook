...
export function TimingPanel({ runId }: Props) {
  ...
  const steps = useMemo(() => [...new Set(spans.map((s) => s.step))].sort((a, b) => a - b), [spans]);
  const spanNames = useMemo(() => [...new Set(spans.map((s) => s.name))].sort(), [spans]);
  ...
  const stepSpans = useMemo(
    () => spans.filter((s) => s.step === displayStep).sort((a, b) => a.wall_start - b.wall_start),
    [spans, displayStep]
  );
                ...
                .map(([name, s]) => (
                  <tr key={name} style={{ cursor: 'default' }}>
                    <td style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{ width: 10, height: 10, borderRadius: 2, background: getColor(name, spanNames), flexShrink: 0 }} />
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                        <div style={{
                          ...
                          height: 6,
                          borderRadius: 3,
                          background: getColor(name, spanNames),
                          minWidth: 2,
                        }} />

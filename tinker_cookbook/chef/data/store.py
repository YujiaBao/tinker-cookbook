...
from tinker_cookbook.storage import Storage, storage_join
...
class RunStore:
    ...
    def storage_for(self, run_id: str) -> Storage:
        """Get the storage that contains a specific run. Falls back to primary."""
        return self._run_storage.get(run_id) or self._storages[0]

    def refresh_runs(self) -> list[RunInfo]:
        """Re-scan all storages for runs. Clears reader caches."""
        all_runs: dict[str, RunInfo] = {}
        self._run_storage.clear()
        self._metrics_readers.clear()
        self._config_readers.clear()
        self._rollout_readers.clear()
        self._logtree_readers.clear()
        self._timing_readers.clear()
        self._eval_readers.clear()

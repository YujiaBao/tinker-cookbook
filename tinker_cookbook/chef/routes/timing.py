...
def create_router(store: RunStore) -> APIRouter:
    ...
    async def get_timing_tree(run_id: str, step: int) -> dict[str, Any]:
        """Get a hierarchical span tree for a specific step.

        Spans are nested by time containment: a child span's time range
        is fully within its parent's. This reveals the call hierarchy
        (e.g., sampling → group_rollout → policy_sample).
        """
        run = store.get_run(run_id)
        if run is None:
            raise HTTPException(status_code=404, detail=f"Run '{run_id}' not found")
        ...
        return reader.build_span_tree(step)
